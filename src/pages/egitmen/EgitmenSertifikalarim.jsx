...
export default function EgitmenSertifikalarim({ user }) {
  ...
  const fetchCertificates = async () => {
    try {
      ...
      console.error("Eğitmen sertifika hatası:", err);
      setCertificates([]);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = certificates
    .filter((c) => !c.sertifika_pasif_mi && c.aktif_mi !== false)
    .filter((c) =>
      (c.title || "").toLowerCase().includes(search.toLowerCase())
    );

  const monthlyCount = certificates.filter((c) => {
  ...
  <div className="flex min-h-screen bg-[#F8FAFC]">
    ...
    <main className="flex-1 bg-[#F8FAFC] min-h-screen">
      ...
      <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-20">
        <div>
          <p className="text-xs font-black tracking-[3px] text-red-600 mb-1">
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-slate-400 font-semibold max-w-sm leading-7">
              {search
                ...
                : "Eğitmen öğrenme alanındaki eğitimleri tamamlayarak sertifika kazanabilirsin."}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filtered.map((cert, i) => (
              <CertCard
                key={cert.id || i}
                cert={cert}
                userName={user?.name || cert.userName || "Eğitmen"}
      ...
      {selected && (
        <CertModal
          cert={selected}
          userName={user?.name || selected.userName || "Eğitmen"}
