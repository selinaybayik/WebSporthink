...
export default function EgitmenOgrenmeAlanim({ user }) {
  ...
  const loadTrainings = async () => {
    ...
    try {
      ...
      console.log("Eğitmen öğrenme alanı API hatası:", error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTrainingPress = async (item) => {
    if (item.status === "new") {
      try {
        ...
        await loadTrainings();
        navigate(`/egitmen/ogrenme-detay/${item.id}`);
      } catch (error) {
    ...
    <main className="flex-1 bg-[#F8FAFC] min-h-screen">
      ...
      <section className="p-10">
        <div className="flex items-start justify-between mb-8">
          <div>
            <p className="text-xs font-black tracking-[3px] text-red-600 mb-3">
              EĞİTMEN ÖĞRENME ALANI
            </p>

            <h1 className="text-4xl font-black text-slate-950 flex items-center gap-3">
              Öğrenme Alanım <span>📚</span>
            </h1>

            <p className="text-slate-500 font-semibold mt-2">
              Bu hafta{" "}
              <span className="text-red-600 font-black">{completedXp} XP</span>{" "}
              topladın. Sana atanan veya keşfedebileceğin eğitimleri buradan
              takip edebilirsin.
            </p>
          </div>
...
function StatCard({ title, value, icon: Icon, color }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] p-7 border border-slate-200 shadow-sm flex items-center gap-6 hover:shadow-md transition">
      <div
        className={`w-16 h-16 rounded-3xl flex items-center justify-center ${styles[color]}`}
      >
        <Icon size={30} />
      </div>
