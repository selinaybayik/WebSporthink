...
export default function EgitmenOgrenmeDersDetay({ user }) {
  ...
  const loadLesson = async () => {
    try {
      ...
      console.log("Eğitmen ders yükleme hatası:", error.message);
      setLessonData(null);
    } finally {
      setLoading(false);
    }
  ...
  const goToDetail = () => {
    navigate(`/egitmen/ogrenme-detay/${training.id}`);
  };

  const goToModule = (targetModuleId) => {
    navigate(`/egitmen/ogrenme-ders/${training.id}/${targetModuleId}`);
  };
