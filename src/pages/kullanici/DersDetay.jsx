...
export default function DersDetay({ user, setUser }) {
  ...
  const loadLesson = async () => {
    try {
      ...
      console.log("Ders yükleme hatası:", error.message);
      setLessonData(null);
    } finally {
      setLoading(false);
    }
  ...
  const goToDetail = () => {
    navigate(`/user/egitim-detay/${training.id}`);
  };

  const goToModule = (targetModuleId) => {
    navigate(`/user/ders-detay/${training.id}/${targetModuleId}`);
  };
    ...
    return (
      <div className="min-h-screen bg-slate-100 p-10">
        ...
        <div className="max-w-4xl mx-auto bg-white rounded-[2.5rem] p-14 text-center border border-slate-200 shadow-xl relative overflow-hidden">
          ...
          <p className="text-slate-400 tracking-[4px] font-black mb-8">TEBRİKLER!</p>
          ...
          <p className="text-slate-500 italic mb-16">programı için verilmiştir.</p>
          ...
          <div className="flex justify-center gap-14 mb-14">
            <div>
              ...
              <p className="text-slate-400 font-black text-xs tracking-widest mt-2">TARİH</p>
            </div>
            <div className="w-px bg-slate-200" />
            <div>
              <p className="text-2xl font-black text-slate-950">{certificateCode}</p>
              <p className="text-slate-400 font-black text-xs tracking-widest mt-2">KOD</p>
            </div>
          </div>
  ...
  if (screenMode === "quiz" && currentQuestion) {
    return (
      <div className="min-h-screen bg-slate-50">
        ...
        {/* ✅ Modal quiz ekranında da gösteriliyor */}
        {modalType && (
          <ResultModal
            modalType={modalType}
            score={score}
  ...
  return (
    // ✅ DÜZELTME: setUser prop'u eklendi
    <KullaniciLayout user={user} setUser={setUser} activePath="/user/egitimler" searchPlaceholder="Ders içinde ara...">
      <header className="h-20 border-b border-white/10 px-10 flex items-center">
        <button
          onClick={goToDetail}
          className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center mr-5"
        >
          <ArrowLeft size={22} />
        </button>
      ...
      <main className="p-10 grid grid-cols-3 gap-8">
        {/* VIDEO ALANI */}
          ...
          <div className="bg-slate-900 rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl">
            {module.videoUrl ? (
              ...
              <div className="aspect-video bg-slate-800 flex flex-col items-center justify-center">
                ...
                <p className="text-white/50 font-bold">Video bağlantısı eklenmedi</p>
              </div>
            )}
          </div>

          <div className="mt-8 bg-white text-slate-950 rounded-[2rem] p-8">
            <p className="text-slate-400 font-black text-xs tracking-[3px] mb-4">DERS BİLGİSİ</p>
            <h2 className="text-2xl font-black mb-4">{module.title}</h2>
            <p className="text-slate-500 font-semibold leading-8">
              {module.content || "Sporthink Akademi ders oynatma alanı. Modülü bitirerek ilerleyebilirsin."}
            </p>
        ...
        <aside className="space-y-6">
          <div className="bg-white text-slate-950 rounded-[2rem] p-7">
            <p className="text-slate-400 font-black text-xs tracking-[3px] mb-4">MODÜL GEÇİŞİ</p>
          ...
          {resources.length > 0 && (
            <div className="bg-white text-slate-950 rounded-[2rem] p-7">
              <p className="text-slate-400 font-black text-xs tracking-[3px] mb-4">EĞİTİM KAYNAKLARI</p>
              <div className="space-y-3">
                {resources.map((resource) => (
                  <button
                    ...
                    className="w-full flex items-center gap-4 p-4 rounded-2xl bg-blue-50 border border-blue-100 text-left"
                  >
                    <div className="w-11 h-11 rounded-2xl bg-white flex items-center justify-center text-blue-600">
                      {resource.type === "PDF" ? <FileText size={20} /> : <Headphones size={20} />}
                    </div>
      ...
      {/* ✅ DÜZELTME: Modal video ekranında da çalışıyor */}
      {modalType && (
        <ResultModal
          modalType={modalType}
          score={score}
...
// RESULT MODAL
// =====================
function ResultModal({ modalType, score, total, onClose, onCertificate, onRetry, onVideo }) {
        ...
        {success ? (
          <>
            <button
              ...
              className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black mb-3"
            >
              Sertifikayı Göster
            </button>
            <button
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/60 flex items-center justify-center p-8">
      <div className="bg-white w-full max-w-6xl rounded-[2rem] p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-black text-slate-950">{resource.title || "Kaynak"}</h2>
          <button
            onClick={onClose}
        ...
        <iframe
          ...
          src={`https://docs.google.com/gview?embedded=true&url=${encodeURIComponent(resource.url)}`}
