...
import { ArrowLeft, Award, BookOpen, Flame, Medal, Trophy, User } from "lucide-react";
        ...
        {loading ? (
          ...
          <section className="p-10">
              ...
              <div className="relative flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-slate-300 font-bold mt-2">
                    {pUser?.departman || "Departman yok"} · {pUser?.rol || "Kullanıcı"}
                  </p>
            ...
            <div className="grid grid-cols-3 gap-6 mb-8">
              <StatCard icon={<BookOpen />} label="Tamamlanan Eğitim" value={profile?.stats?.completedTrainings || 0} />
              <StatCard icon={<Award />} label="Sertifika" value={profile?.stats?.certificates || 0} />
              <StatCard icon={<Flame />} label="Aktif Seri" value={pUser?.streak || 0} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="bg-white border border-slate-200 rounded-[2rem] p-8">
                <h3 className="text-xl font-black text-slate-950 mb-5 flex items-center gap-2">
                  <Trophy className="text-red-600" /> Tamamladığı Eğitimler
                </h3>

                {profile?.completedTrainings?.length ? (
                  <div className="space-y-4">
                    {profile.completedTrainings.map((item) => (
                      <div key={item.id} className="bg-slate-50 rounded-2xl p-4 flex justify-between">
                        <p className="font-black text-slate-900">{item.title}</p>
                        <p className="font-black text-red-600">{item.xp || 0} XP</p>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-slate-400 font-bold">Henüz tamamlanan eğitim yok.</p>
                )}
              </div>

              <div className="bg-white border border-slate-200 rounded-[2rem] p-8">
                <h3 className="text-xl font-black text-slate-950 mb-5 flex items-center gap-2">
                  <Medal className="text-red-600" /> Rozetleri
                </h3>

                {profile?.badges?.length ? (
                  <div className="grid grid-cols-2 gap-4">
                    {profile.badges.map((badge) => (
                      <div key={badge.id} className="bg-red-50 rounded-2xl p-4">
                        <p className="font-black text-red-700">{badge.title}</p>
                  ...
                  <p className="text-slate-400 font-bold">Henüz rozet yok.</p>
                )}
              </div>
            </div>
          </section>
...
function StatCard({ icon, label, value }) {
  return (
    <div className="bg-white border border-slate-200 rounded-[2rem] p-7">
      ...
      <p className="text-slate-400 text-xs font-black tracking-[3px]">{label}</p>
